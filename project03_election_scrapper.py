...
project03_election_scrapper.py: třetí projekt do Engeto Online Python Akademie
...
#== Vytvoření hlavičky a prázdného csv souboru, který ji obsahuje ==#

def vytvoreni_hlavicky_csv():
    ...
    odpoved_obec_hlavicka = requests.get(list(ziskani_url_obci().values())[0]).text
    ...
    #finální verze csv hlavičky
    csv_hlavicka = csv_hlavicka_eng + politicke_strany

    # vytvoření csv a zápis hlavičky
...
def nacitani_dat_do_csv():
    ...
    for obec_url, obec_kod in zip(ziskani_url_obci().values(), ziskani_url_obci().keys()):
    ...
    logging.info("CSV soubor vytvořen, program ukončen.")


#== SPUŠTĚNÍ FUNKCÍ ==#

vytvoreni_hlavicky_csv()
nacitani_dat_do_csv()